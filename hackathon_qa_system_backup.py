...
class HackathonQASystem:
    ...
    def check_document_exists(self, document_url: str) -> bool:
        ...
        try:
            ...
            # Try to query with any simple text and filter by document_id
            # If we get results, the document exists
            test_results = query_embeddings(
                query_text="policy document insurance",  # Generic query
...
Answer:"""
    ...
    def answer_questions_batch(self, questions: List[str]) -> List[str]:
        ...
        try:
            ...
            # Collect all relevant context for all questions
            all_context_chunks = []
            for question in questions:
                try:
                    retrieval_results = query_embeddings(
                        ...
                        index_name=self.index_name,
                        namespace=self.namespace,
                        n_results=5  # Increased for more comprehensive context
                    )
                    ...
                    if retrieval_results and 'matches' in retrieval_results:
                        for match in retrieval_results['matches']:
                            if 'metadata' in match and 'text' in match['metadata']:
                                ...
                                if chunk_text not in all_context_chunks:  # Avoid duplicates
                                    all_context_chunks.append(chunk_text)
                                    
                except Exception as e:
                    print(f"⚠️  Warning: Error retrieving context for question: {e}")
                    continue
            ...
            context = "\n\n".join(all_context_chunks[:15])  # Increased context size for better answers
            
            # Create batch prompt with all questions
            questions_text = "\n".join([f"{i+1}. {q}" for i, q in enumerate(questions)])
            
            prompt = f"""You are an expert insurance policy analyst specializing in the National Parivar Mediclaim Plus Policy. Answer ALL questions based ONLY on the provided context from the insurance policy document.
...
Instructions:
1. Provide detailed and comprehensive answers based strictly on the policy document context
2. Include specific numbers, percentages, time periods, and conditions exactly as mentioned in the policy
3. For waiting periods, grace periods, and coverage limits, provide exact values with units (days, months, years, percentages)
4. When mentioning benefits or coverage, include any relevant conditions, exclusions, or sub-limits
5. If a question asks about coverage, explain both what is covered and any important conditions or limitations
6. For questions about definitions (e.g., "Hospital"), provide the complete definition as stated in the policy
7. If the context doesn't contain sufficient information for a specific question, state "Information not available in the provided document"
8. Format your response as a numbered list matching the question numbers exactly
9. Each answer should be complete and self-contained, providing all relevant details from the policy
10. Use professional insurance terminology as found in the source document

Provide comprehensive and accurate answers for all questions:"""
            ...
            print(f"✅ All {len(questions)} questions answered in batch")
            return answers
    ...
    def process_hackathon_request(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"⚡ Average time per question: {processing_time/len(questions):.2f}s")
